...
// Safely load lovable-tagger only in dev environments.
// It is not installed in Docker / IBM Cloud production builds.
function loadComponentTagger(): (() => Plugin) | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    return require('lovable-tagger').componentTagger as () => Plugin;
  } catch {
    return null;
  }
}
...
export default defineConfig(({ mode }) => {
  const taggerFn = mode === 'development' ? loadComponentTagger() : null;
  ...
  return {
    ...
    resolve: {
      alias: {
        ...
        "react-dom/jsx-dev-runtime": path.resolve(__dirname, "node_modules/react/jsx-dev-runtime"),
      },
